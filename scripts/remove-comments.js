...
function stripComments(code) {
  ...
  while (i < len) {
    ...
    // handle string toggles if not escaping
    if (!inRegex && !inTemplate && ch === '"' && !inSingle && prev !== '\\') {
      inDouble = !inDouble;
      out += ch; i++; prev = ch; continue;
    }
    if (!inRegex && !inTemplate && ch === "'" && !inDouble && prev !== '\\') {
      inSingle = !inSingle;
      out += ch; i++; prev = ch; continue;
    }
    ...
    // If inside any string/template, copy char and continue
